...
then
  ...
  valgrind $flags ./fileman -b 1>/dev/null 2>>valgrind.out
  valgrind $flags ./fileman -a 1>/dev/null 2>>valgrind.out
  valgrind $flags ./fileman -s 1>/dev/null 2>>valgrind.out
  valgrind $flags ./fileman -e 1>/dev/null 2>>valgrind.out
...
mv valgrind.out fred 2>/dev/null
rm *.out *.diff .*.out
rm -rf data.dir
mv fred valgrind.out 2>/dev/null

echo "The grading system uses significantly larger files than the teaching"
echo "servers. If your code does not handle large files well, you may fail"
echo "some tests. You will also fail tests if you fail to close any of the"
echo "files you opened."
echo ""
