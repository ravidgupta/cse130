...
{
  /*
   * checks for 0 cases then adds to the list when they sleep
   */
  if (ticks > 0){
    // printf("I");
    int64_t start = timer_ticks();
    struct thread *cur = thread_current();
    cur->sbtime = start + ticks;
    ASSERT (intr_get_level () == INTR_ON);
    list_push_back(&sleepingList, &cur->nElem); //friend tole me to use this fn
    intr_disable();
    thread_block();
    intr_enable();
  }
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  thread_tick ();
  /*
   * Loops through the list and wakes up thread that need it
   */ 
  struct list_elem *i = list_begin(&sleepingList);
  while (i != list_end(&sleepingList)){
  //  printf("f");
    struct thread *t = list_entry(i, struct thread, nElem);
  //  printf("I");
    int checkTime = t->sbtime;
    if (ticks >= checkTime){
      i = list_remove(i);
      thread_unblock(t);
    }
